...
class LeadView(APIView):
    def post(self, request):
        ...
        # ✅ Step 2: Get the Organization linked to the user (via created_by)
        ...
        data = request.data
        data["organization"] = organization.id  # Assign the organization ID
    ...
    def _send_lead_to_users(self, lead, organization):
        ...
        for user in matching_users:
            ...
            # ✅ Save assignment with lead_link
            try:
...
class LeadDecisionView(APIView):
    def post(self, request, lead_id, token):
        ...
        # ✅ Extract user from Supabase instead of request.user.id
        response = verify_supabase_token(request)
        if response.status_code != 200:
            return response
        ...
        # ✅ Find LeadAssignment for this user
    ...
    def reassign_lead(self, lead):
        ...
        if next_user:
            ...
            # ✅ Create a new LeadAssignment for the new user
            LeadAssignment.objects.create(
                lead=lead,
                user=next_user,
                assigned_at=timezone.now(),
            ...
            print(f"Lead reassigned to {next_user.email}: {lead_link}")  # ✅ Debugging
