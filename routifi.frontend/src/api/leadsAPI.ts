...
export const getLeadsByOrgSlug = async (slug: string) => {
  ...
  try {
    ...
    throw error.response?.data || "Failed to fetch leads | Owner access required";
  }
};
// Update Lead (Only if NOT assigned)
export const updateLead = async (slug: string, leadId: string, leadData: any) => {
    if (!(await ensureAuth())) {
      window.location.href = "/sign-in";
      throw new Error("User not authenticated");
    }
    ...
    try {
      const response = await api.put(`/app/org/${slug}/${leadId}/`, leadData);
  ...
  export const deleteLead = async (slug: string, leadId: string) => {
    ...
    try {
      const response = await api.delete(`/app/org/${slug}/${leadId}/`);
      return response.data;
    } catch (error: any) {
